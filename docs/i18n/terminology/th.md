...
| flag | ปักธง | Literally *to plant a flag*. It is a disposition, not an alarm, and it is deliberately NOT the phrasing used for marking something as needing review — that action is built on เครื่องหมาย (*a mark*) instead, which matters because flagging CLEARS the needs-review state. Where an LQA check *flags* something in the loose English sense, this locale writes รายงาน (*reports*) so the term is not diluted; see the ignore row. |
...
| review queue | คิวตรวจทาน | Built on the same ตรวจทาน as the needs-review state, so a reader sees one feature and not two: the queue is where items in that state are listed. |
| back-translation | การแปลกลับ | The established Thai term of art. Deliberately not built on the revert word, which would name an action the product does not offer — this is a read-only reference panel. |
...
| notification severity | ระดับ | The same word as the LQA severity row, and its warning value is the identical คำเตือน, which the shared lexicon requires. The three values are ข้อมูล, คำเตือน and วิกฤต. |
...
| pass rate | อัตราผ่าน | Kept clear of สำเร็จ, which `config:health.successRate` uses for a completely different metric (provider request success) in the same product. |
...
| constant | คงที่ | *Fixed / unchanging*. Kept apart from อ่านอย่างเดียว — the read-only flag two columns away in the same table — which is the near-miss the shared lexicon names. Thai has no case, so the column header and the lowercase chip are the same string. |
| match | จุดที่ตรงกัน | The noun — literally *the spot that matches* — for a place a glossary term was found; it heads the Matches panel and its counts. The verb stays on the same root: ตรงกัน for *matches* in a routing condition or a search, จับคู่ for the act of matching. Never คำศัพท์, the glossary-term word: a term is what is searched for, a match is where it turned up. Counted with the classifier จุด. |
...
| element | ธาตุ | The group heading only. The seven elemental names themselves stay in Latin: Genshin Impact ships no Thai client, so there is no in-game Thai name to adopt, and the shared lexicon says to leave them where the game leaves them. All seven are under the identical-value check's threshold for an unspaced locale, so none needs an allowlist entry. |
