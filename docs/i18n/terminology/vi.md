...
| log | nhật ký | The live server-log panel and its lines. **A single line needs its own word here and it is built from the same root**, as this row permits: *dòng nhật ký* (a log line), used at `console:empty` and `console:membersNotShown`, because bare *mục* is reserved for a content **entry** and this app counts those constantly. The panel header `console:title` is *BẢNG NHẬT KÝ* rather than bare *NHẬT KÝ*: the sidebar is always painted and carries **Nhật ký thay đổi** (changelog), so a bare heading would have sat one word away from it. Not *hoạt động*, which is reserved for **Activity** — that is the reservation this row exists to hold from the other side. |
