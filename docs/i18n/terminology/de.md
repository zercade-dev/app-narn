...
| entry | Eintrag | n., masc., pl. *Einträge*. The counting unit of the whole app. `config:routing.simpleHint` says *string* in English and is rendered with this word, per the canonical-term rule. Do not reuse for a translation-memory record without a qualifier — `config:tm.clearAllSuccess_one` writes it out in full. |
...
| run | Durchlauf | n., masc., pl. *Durchläufe*. Never a verb. Bare *Lauf* was rejected: on its own it reads as running/jogging, and *Durchlauf* is the ordinary German word for one execution of a process. |
| Activity |  |  |
...
| judge | bewerten | Verb, evaluative sense; the deverbal noun *Bewertung* names the run type in `config:requestTimeoutDescription`. Never *richten*/*Richter*. No noun "der Judge" is introduced. |
...
| suggestion |  |  |
| discard | Verwerfen | Sense 1 only — the unsaved-edits button beside Save (`config:discard`). The reject-a-proposal sense needs a second word and is not decided yet; do not extend this one to it. `config:discardUntranslatable` is a **third** sense again — omitting entries from an export — and is written as "weglassen". |
...
| approve | freigeben | Verb; the only route into the Translation Memory (`config:tm.browserEmpty`, which uses the nominalized infinitive rather than a 2nd-person form). Distinct from *anwenden* (apply), *als geprüft markieren* (mark reviewed) and *speichern* (save). |
