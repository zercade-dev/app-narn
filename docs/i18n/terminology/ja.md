...
| translator context | 翻訳者コンテキスト | Batch 1. `config:includeContext` is 「翻訳者コンテキストの列を含める」: the bare 「コンテキスト列」 would collide with コンテキストウィンドウ (`config:models.confidenceReason.prompt-near-context`), which the lexicon forbids. The shared コンテキスト root is licensed because the heads differ. |
...
| needs review |  |  |
| flag |  | Not met in the review-queue sense. The LQA descriptions' English verb *flag* (「Flags translations…」) is **not** this term: it means the check reports an issue, and is rendered 検出します (`config:lqa.checks.overflow.description`). Keep whatever word *flag* takes in batch 3/4 clear of 検出. |
...
| quality gate | ゲート | Batch 1. English also says bare *the gate* once *quality checks* has been said, and Japanese does the same: `config:lqa.description` is 「品質チェックは…ブロッキングの問題はゲートを通過できず…」. 門／関門 are the physical-door readings the lexicon warns about. |
