...
- **`erase`** — **not applied, and deliberately not improvised.** Its resolution folds the
  distinction into "the frozen `delete` row", and there is no `delete` row in this lexicon —
  there never has been. The fact it records is real (`glossary:confirmPushReplaceTitle`
  destroys rows in the DeepL glossary, a destination the product does not own, so a locale
  rendering it with its ordinary delete word loses the warning that the destruction is not
  local), but it has no home the resolution actually named, and inventing one would be a
  decision this queue never took. **This is the one row still owed a resolution** — either a
  `delete` term or an `english-review-notes.md` row, decided by whoever runs the next
  resolution.
