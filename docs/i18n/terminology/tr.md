...
| credential vault | kasa | **The term is “kimlik bilgisi kasası”; “kasa” is only the clip.** Bare “kasa” is licensed *only* where the surrounding string already establishes credentials — which is every `config` site (`credentialsVaultLockedChip`, `credentialsUnlockButton`, `credentialsVaultLocked`, `credentialsMissing`). **Cold, standalone namings must use the full form**, starting in **batch 2** with `strings:guide.topicVault` (“Credential Vault”), and then `vault:statusLabel` in batch 4: a bare “kasa” there is the safe/strongbox reading the frozen lexicon's Not-list bans. All four required phrasings read naturally on this stem: kasa kilitli / kasanın kilidini aç / kasa parolası / kimlik bilgisi kasası. **Why the Rendering column holds the clip, and what that means for every row in this file:** the citation guard (`scripts/check-lexicon-citations.mjs`) requires each significant word of this column to be attested in the shipped `tr` corpus, and “kasası” is not attested until a batch actually ships the full form — so the guard structurally **forbids recording a term's full form ahead of the batch that ships it**, and every row here is biased toward the clipped, batch-1-shaped wording by construction. Read this column as “the longest form shipped so far”, not as “the term”, and promote a row the moment your batch ships the fuller one. Bending a string to make a citation attestable is the wrong fix and was refused here. |
